...
for i in range(3):
    NoOptionPicked = True
    InstructStr = "Puli (Tiger) Player! Place down " + str(3 - i) + " more Puligal!"
...
while(checkGame(LambList, TigerList)):
    if (PlayerTurn == 1): #Lamb's Turn
        ...
        if (LambsLeft > 0): #Need to Place Down Lambs
            NoOptionPicked = True
            InstructStr = "Aadu Player! Place down a Aadu, you have " + str(LambsLeft) + " remaining"
            InstructText.setText(InstructStr)
            while (NoOptionPicked):
                CP = win.getMouse()
                CL = getCircleLoc(CP)
        ...
        TurnComplete = False
        InstructStr = "Puli Player! Move a Puli or jump over a Aadu!"
        InstructText.setTextColor('black')
        InstructText.setText(InstructStr)
        while (TurnComplete == False):
            NoOptionPicked = True
