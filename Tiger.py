...
InstructText.setTextColor('orange')
...
for i in range(3):
    NoOptionPicked = True
    InstructStr = "Tiger Player! Place down " + str(3 - i) + " more tiger(s)!"
...
while(checkGame(LambList, TigerList)):
    if (PlayerTurn == 1): #Lamb's Turn
        InstructText.setTextColor('red')
        if (LambsLeft > 0): #Need to Place Down Lambs
            NoOptionPicked = True
            InstructStr = "Lamb Player! Place down a Lamb, you have " + str(LambsLeft) + " remaining"
            InstructText.setText(InstructStr)
            while (NoOptionPicked):
                CP = win.getMouse()
                CL = getCircleLoc(CP)
            ...
            TurnComplete = False
            InstructStr = "Lamb Player! Move one of your lambs!"
            InstructText.setText(InstructStr)
        ...
        TurnComplete = False
        InstructStr = "Tiger Player! Move a Tiger or jump over a Lamb!"
        InstructText.setTextColor('orange')
        InstructText.setText(InstructStr)
        while (TurnComplete == False):
            NoOptionPicked = True
...
if (len(LambList) + LambsLeft < 4):
    InstructText.setText('Tiger Player Wins!')
else:
    InstructText.setText('Lamb Player Wins!')
